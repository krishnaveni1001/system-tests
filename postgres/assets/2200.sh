  ...
  ssh -i /keys/id_rsa $REMOTE_SSH_USER@$REMOTE_DB_HOST << EOF
    time \
      psql \
        ...
        -d ${REMOTE_DB_DB} \
        -c "COPY (SELECT * FROM $TABLE WHERE company_id = (select company_id from cs_company where client_view_id = (select client_view_id from cs_environment where environment_type = '$ENVIRONMENT_TYPE') and level_ind = '1')) TO STDOUT" > /tmp/$FILE_NAME.tsv
...
EOF
  ...
  psql \
    -d local \
    -c "create function my_temp_insert_trigger_function() returns trigger as 'BEGIN new.company_id = (select company_id from cs_company where client_view_id = (select client_view_id from cs_environment where environment_type = ''$ENVIRONMENT_TYPE'') and level_ind = ''1''); return new; END' language 'plpgsql'"
  psql \
    -d local \
...
copyData cs_vessel E-ONEY
copyData cs_service_string E-ONEY

copyData cs_vessel E-MAEU
copyData cs_service_string E-MAEU

copyData cs_vessel E-CMDU
copyData cs_service_string E-CMDU

copyData cs_vessel E-ANLC
copyData cs_service_string E-ANLC