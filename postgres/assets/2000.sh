#!/bin/ash
# This function copies data from cs_ext_tl_surcharge_location_master and inserts
  ...
  ssh -i /keys/id_rsa $REMOTE_SSH_USER@$REMOTE_DB_HOST << EOF
    time \
      psql \
        ...
        -d ${REMOTE_DB_DB} \
        -c "COPY (SELECT * FROM $TABLE WHERE client_view_id=(SELECT client_view_id FROM cs_environment WHERE environment_type = '$ENVIRONMENT_TYPE')) TO STDOUT" > /tmp/$FILE_NAME.tsv
...
EOF
  ...
  psql \
    -d local \
    -c "create function my_temp_insert_trigger_function() returns trigger as 'BEGIN new.client_view_id = (select client_view_id from cs_environment where environment_type = ''$ENVIRONMENT_TYPE''); return new; END' language 'plpgsql'"
  psql \
    -d local \
...
copyData cs_ext_tl_surcharge_location_master E-ONEY
copyData cs_carrier_commodity_master E-ONEY
copyData cs_ext_location_master E-ONEY

copyData cs_ext_tl_surcharge_location_master E-MAEU
copyData cs_carrier_commodity_master E-MAEU
copyData cs_ext_location_master E-MAEU

copyData cs_ext_tl_surcharge_location_master E-CMDU
copyData cs_carrier_commodity_master E-CMDU
copyData cs_ext_location_master E-CMDU

copyData cs_ext_tl_surcharge_location_master E-ANLC
copyData cs_carrier_commodity_master E-ANLC
copyData cs_ext_location_master E-ANLC